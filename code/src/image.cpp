...
Vector3f Image::textureGetPixel(float x, float y, bool filtered) {
    ...
    Vector3f topright = GetPixel(xint + 1, yint);
    Vector3f botleft = GetPixel(xint, yint + 1);
    Vector3f botright = GetPixel(xint + 1, yint + 1);
    return (1 - ycopy + yint) * ((1 - xcopy + xint) * topleft + (xcopy - xint) * topright) + (ycopy - yint) * ((1 - xcopy + xint) * botleft + (xcopy - xint) * botright);
}
